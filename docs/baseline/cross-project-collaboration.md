...
| 评估 / 采纳 | 目标项目 PM / Architect | **Owner + 参谋长（框架维护身份）** |
...
- **评估 / 采纳 / 落地**：仅 **Owner + 参谋长**（参谋长以框架维护身份在 agent-workflow 目录落地，BCR-009）。`agent-workflow` 须先登记进 `PROJECTS.md`（定位：工作流真源，只承接 BCR）才受理提报。
...
组织架构定位为**一人公司 AI 组织操作架构：指挥官—参谋长制（薄公司）**。生态参与者分**两类 + 一个场所**，各有固定性质、边界与跨仓写权限。**公告板是场所不是人，不发头衔；结构跟着物理走，一个头衔辖明确的物理目录（可为多个）+ 会话，无座位不发头衔**。工作流组（框架真源）的维护身份并入参谋长（BCR-009）：agent-workflow 不再是独立头衔/会话，降为**参谋长的第二工作目录**——裁一个头衔、目录并入现有头衔，正是「无座位不发头衔」的强化。

```text
指挥官（Owner / CEO）—— 唯一决策者
├── 参谋长（生态根会话 + agent-workflow 维护 —— 打理事务 + 造/维护 SOP）
...
|------|------|----------------|------------|------------|
| 指挥官 | Owner 本人，唯一决策者 | — | 一切拍板；半自动阶段亲自进各组行使项目级 PM 职权 | — |
| 项目组（开发型下游） | 业务开发项目，接入 dev 工作流（用 SOP 作战） | 是 | 仓内按角色/门禁开发；组内 PM/Architect/Developer/DevOps，无组长 | 向公告板登记 REQ/BCR/元信息台账行；不改 baseline、不改别的下游 |
| 参谋长（生态根会话 + agent-workflow 维护） | 生态事务打理者 + 框架真源维护者，参谋不决策 | 否（事务/维护身份，General 直接维护制品，不走角色/Review 门禁） | ① 立项受理+建壳 ② 维护公告板（PROJECTS 同步、台账勾选、BCR 全周期推进）③ 维护 agent-workflow baseline/模板/入口 + 评估·采纳·落地 BCR（第二工作目录）④ 工作流回流执行（改完框架顺手跑 sync 脚本）⑤ 陪指挥官调研 + 生态导航索引；不做任何项目的 PRD/架构/开发/部署；不替指挥官决策；不进组扮演工作流角色 | 对公告板**字段级白名单直写**（见下）；直写 agent-workflow baseline/模板/入口（第二工作目录）；对下游只准经 `sync-downstream.sh` 同步框架文件，禁止手改；**例外**：立项一次性建壳、框架回流执行 |
| 公告板（场所） | 跨项目边界事实 single source（契约/REQ/BCR/状态/项目目录），纯 Markdown，**不是人** | 否（撤销专职守门会话） | 需求池 / BCR 池 / 状态台账 / 契约 / 决策的物理载体；各组授权直写协议不变 | —（场所不写东西，由参与者写） |
...
3. BCR 池**全周期推进**（评估记录 / 采纳结论 / 真源落地 commit / 回流清单与状态）——参谋长兼 BCR 评估落地方（BCR-009）；
...
1. **结构跟着物理走**：一个头衔辖明确的物理目录（可为多个）+ 会话；无座位不发头衔，任何没有座位的头衔都是管理戏服，禁止发放。参谋长辖生态根 + agent-workflow 两个目录（框架维护并入后）——目录都在、只是不再各自发独立头衔，仍是「结构跟着物理走」。
...
| 目标 ＼ 写方 | 项目组（开发型下游） | 参谋长（含框架维护） | 公告板（场所） |
|-------------|---------------------|---------------------|---------------|
| `baseline/`（agent-workflow） | ❌（只提 BCR） | ✅（第二工作目录，直写） | ❌ |
| 公告板 | ✅（REQ/BCR/元信息台账行） | ✅（白名单字段 + BCR 池评估落地 + 自身元信息行） | — |
| 生态索引文件（根） | ❌ | ✅ | ❌ |
| 下游业务仓 | ✅（自己） | ✅（仅经 sync 脚本） | ❌ |
...
- **真源**：项目元信息结构化单一真源是公告板项目目录（`PROJECTS.md`）；生态索引是其导航视图，照真源同步、不自由发挥。
- **载体**：变更登记在公告板状态文件（`STATUS.md`）「元信息变更台账」——字段 `项目 | 字段 | old | new | 来源（commit/迭代） | PROJECTS 已同步 | 生态索引已同步`。
- **两方接力**：
  1. 子项目迭代关闭检查发现变更 → 登记一行（两「已同步」列留空）；
  2. 参谋长照 new 值一站式同步：改 `PROJECTS.md` + 订正根索引 + 勾两「已同步」列（白名单第 1/2/4 条权限内，立项登记也由参谋长直落）。
- **真源可疑时停机**：参谋长发现各仓实况与 `PROJECTS.md` 不一致且台账无在途行时，**不得**按实况直接改索引、**不得**同步到过期真源，须输出待登记台账行通知变更项目先登记台账行，再同步。
...
> 立项编排归属（多项目生态内）：新项目的「受理 + 建空项目架构」——建壳、跑 `install-downstream.sh`、首推远端——由**参谋长**（生态根会话）一条龙承接（白名单第 4 条权限内）；meta 仍落 coordination `PROJECTS.md`，Bootstrap + 项目定位仍在新项目会话。下面步骤是新项目会话视角的复用步骤：参谋长跑完第 1 步（install-downstream + 立项登记）后，新项目会话从第 2 步（填 `project-context.md`）接续。

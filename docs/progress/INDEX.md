...
- 当前阶段：BCR-013（会话「角色+迭代」标识）Owner 已拍前置门=先探底 + 契约落点=contracts/；②③④挂起待第 0 档探底数据
- 阻塞项：无（球在 workboard：第 0 档解析侧探底归其迭代执行）
- 下一步入口：第 0 档归 workboard 解析侧迭代（改进 detectRole 扫 assistant 存量，角色维报 Unknown 率 / 迭代维报命中率，各对门槛各判是否进框架）；参谋长待 workboard 探底数据回来复评 ②③④，判定进框架才另起落地游标

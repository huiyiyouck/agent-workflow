...
- **下一步**：P15 继续——U2 CI 门禁（workboard `l1-gates.yml` G1-G5 + 真源轻量 workflow + sync 扩展分发）→ U3 免确认规则改写（S1-S4，含降级条款三件套）→ 下游试运行（覆盖矩阵）→ 晋级评审（≤2 阈值 + 注入关 + 留痕关）。**待办（非阻塞）**：coordination PROJECTS.md agent-workflow 行「当前入口」字段存量过期，顺手订正。（原待办② 瘦身审计已于 2026-07-26 以误报关闭，见「正在做」⚠️。）
- **本轮搁置（明确不做）**：暂无。

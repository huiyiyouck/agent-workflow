...
| WM（工作流管理者） | 角色设计维护、基线规则修改、流程审计、工作流体系完善 | 元流程，操作对象是工作流框架本身 |
...
Bootstrap 初始化、收尾归档、迭代关闭检查不是角色。它们由当前会话 Agent 在用户要求或检测到触发条件时执行，关键结果由用户确认；流程审计由 WM（工作流管理者）执行。工作流加载后，未初始化项目只能建议 Bootstrap，不能因为用户问候或闲聊就自动创建文件。Bootstrap 只安装团队工作台，不等于启动标准迭代；用户未选择角色或工作类型时，可以保持 General（通用助手）。详细规则见 `docs/baseline/mechanisms.md`。
...
1. 将本目录复制到新项目。Codex 项目保留或复制 `AGENTS.md`，Claude Code 项目保留 `CLAUDE.md`；如果同一项目同时使用两种助手，两个入口文件都保留并保持内容一致。

...
Bootstrap 初始化、收尾归档、迭代关闭检查、流程审计不是角色。它们由当前会话 Agent 在用户要求或检测到触发条件时执行，关键结果由用户确认。未初始化项目只能建议 Bootstrap，不能因为用户问候或闲聊就自动创建文件。详细规则见 `docs/baseline/mechanisms.md`。
...
为避免上下文膨胀，Agent 启动时先读 `docs/baseline/runtime.md`，再按场景顺序加载需要的规则文件。默认只读项目事实、进度索引、当前角色手册和相关摘要；标准迭代、非迭代、收尾归档、知识库和模板文件都按触发条件再读取。角色日志过长时按 `docs/baseline/context-policy.md` 摘要归档。
...
4. 如果是空项目，先不要手工创建 `project-context.md` 或 `docs/progress/INDEX.md`，第一次启动时说：`执行 Bootstrap 初始化流程`，由 Bootstrap 创建。
5. Bootstrap 会初始化 `docs/progress/INDEX.md`、`docs/progress/iterations/`、`docs/progress/roles/`。
6. 后续每次启动 Claude Code 时指定角色，例如：`这次以 Architect（架构师）角色工作`。

## 基本原则

- `CLAUDE.md` 只做入口索引，不塞长篇流程。
- `runtime.md` 只做加载路由，不写完整规范。

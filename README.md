...
这套工作流适合复制到 Codex、Claude Code 或其他支持项目级指令文件的 AI 编程助手项目中使用。项目自己的业务背景、技术栈、启动方式和业务边界写入 `docs/baseline/project-context.md`，当前迭代状态写入 `docs/progress/INDEX.md` 和 `docs/progress/iterations/`，通用协作规则保持不变。

> **本仓库性质（真源仓库 / single source of truth）**：本仓库本身的用途是**研究和维护这套工作流**，并作为后续所有项目引用的统一真源。仓库内的 `CLAUDE.md`/`AGENTS.md`、`docs/baseline/`、`docs/templates/` 是被开发、被维护的**制品**，不是约束本仓库会话的操作法律。在本仓库工作时默认以普通助手（General）身份直接编辑这些文件，不进入工作流、不写 `docs/progress/`。工作流框架在本仓库统一修改，下游项目只提 `[基线修正提案]` 带回这里处理。入口文件顶部的 `SOURCE-REPO-ONLY` 块只服务本仓库，复制/同步到下游项目时必须整块删除。
...
> 工作流框架本身（角色设计、基线规则、流程审计、体系完善）由**真源仓库**统一维护，不再设独立的工作流管理角色。下游项目发现问题时写 `[基线修正提案]` 带回真源仓库处理。
...
项目入口文件默认使用 General（通用助手）角色，并只做扁平角色切换路由。Codex 使用 `AGENTS.md`，Claude Code 使用 `CLAUDE.md`；两个入口文件应保持同一套内容，只负责判断是否从 General 切换到工作流角色。
...
1. 将本目录复制到新项目。Codex 项目保留或复制 `AGENTS.md`，Claude Code 项目保留 `CLAUDE.md`；如果同一项目同时使用两种助手，两个入口文件都保留并保持内容一致。**复制后必须删除入口文件顶部的 `SOURCE-REPO-ONLY` 块**（该块只适用于真源仓库）。
...
4. 如果是空项目，先不要手工创建 `project-context.md` 或 `docs/progress/INDEX.md`，第一次需要加载工作流时说：`执行 Bootstrap 初始化流程`，由 Bootstrap 创建工作台目录和进度索引。
5. Bootstrap 会初始化 `docs/progress/INDEX.md`、`docs/progress/iterations/`、`docs/progress/ad-hoc/`、`docs/progress/archive/`、`docs/progress/roles/`，但不会默认创建迭代、角色日志或项目上下文。
6. Bootstrap 后可指定角色或工作类型，例如：`这次以 Architect（架构师）角色工作`；如果暂时不工作，也可以保持 General 继续闲聊。
